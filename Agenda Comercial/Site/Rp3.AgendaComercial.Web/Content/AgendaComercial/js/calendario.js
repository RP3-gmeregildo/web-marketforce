...
$(function () {
    ...
    for (i = 1; i <= 7; i++) {
        toTimePicker($('#desdeJornada1_' + i));
        toTimePicker($('#desdeJornada2_' + i));
        toTimePicker($('#hastaJornada1_' + i));
        toTimePicker($('#hastaJornada2_' + i));
        //var pru = $('#descripcion').attr('disabled');
        //if ($('#descripcion').attr('disabled') == 'true')
        //{

        if (!readOnly) {
            $("#jornada2_" + i).on('ifChecked || ifUnchecked', function () {
                evaluateChecked(this);
            });
            evaluateChecked($("#jornada2_" + i));

            $("#eslaboral_" + i).on('ifChecked || ifUnchecked', function () {
                evaluateCheckedLaboral(this);
            });
            evaluateCheckedLaboral($("#eslaboral_" + i));
        }
        //}    
    }

    toDatePicker($("#group-dias input[fecha]"));
    toTimePicker($("#group-dias input[hora]"));
...
function removerDia(control)
{
    var id = $(control).attr("idDia");
    $("#group-dias #dia" + id).remove();
}
