...
function saveClienteImage() {
    try {
        ...
        rp3Post("/General/Cliente/SaveFoto", { id: idCliente, idContacto: idClienteContactoFoto, fileName: fileName, x: x, y: y, width: width, height: height }, function (data) {

            if (!idClienteContactoFoto) {
                $("#fotocliente").show();
                $("#fotocliente").attr('src', RP3_ROOT_PATH + data.ThumbnailImagePath);

                $("#Foto").val(data.ImageNamePath);
            }
            else {
                $("#fotocliente" + idClienteContactoFoto).show();
                $("#fotocliente" + idClienteContactoFoto).attr('src', RP3_ROOT_PATH + data.ThumbnailImagePath);
            }
        });
    } catch (err) {
    alert(err.message);
